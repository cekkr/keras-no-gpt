...
def generateModel():
    ...
    input_1 = Input(shape=(tokensBag))  # Replace additional_input_dim with the actual dimension of your second input

    # Define the second input
    input_2 = Input(shape=(nChars))

    # Define the first input branch
    lstm_1 = Dense(tokensBag)(input_1)

    # Define the second input branch
    lstm_2 = Dense(tokensBag)(input_2)
...
def fitSeq():
    ...
    if len(prevSeqChars) > 0:
        ...
        '''
        psb_shape = (seqLen, tokensBag)
        psc_shape = (seqLen, nChars)
        psb = np.zeros(psb_shape)
        psc = np.zeros(psc_shape)

        csb_shape = (seqLen, tokensBag)
        csc_shape = (seqLen, nChars)
